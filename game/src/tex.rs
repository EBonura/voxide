...
/// The second icon set, from tile 80: every item that still showed another
/// block's face (buckets on the iron-ore tile, the torch on lava, flint on
/// fire, potions on the portal...), and the blocks whose hotbar picture was
/// borrowed (furnace, bed, piston, enchanting table, fence, glass, slab,
/// stairs). Placed blocks keep their world faces; only the icon changes.
pub const T_I_BUCKET: u8 = 80;
pub const T_I_WATER_BUCKET: u8 = 81;
pub const T_I_LAVA_BUCKET: u8 = 82;
pub const T_I_TORCH: u8 = 83;
pub const T_I_FLINT: u8 = 84;
pub const T_I_BOTTLE: u8 = 85;
pub const T_I_POT_AWKWARD: u8 = 86;
pub const T_I_POT_SPEED: u8 = 87;
pub const T_I_POT_STRENGTH: u8 = 88;
pub const T_I_POT_REGEN: u8 = 89;
pub const T_I_POT_FIRE: u8 = 90;
pub const T_I_BONEMEAL: u8 = 91;
pub const T_I_WIRE: u8 = 92;
pub const T_I_SUGAR_CANE: u8 = 93;
pub const T_I_WHEAT: u8 = 94;
pub const T_I_FISHING_ROD: u8 = 95;
pub const T_I_EMBER_CAP: u8 = 96;
pub const T_I_EMBER_ROD: u8 = 97;
pub const T_I_WAILER_TEAR: u8 = 98;
pub const T_I_MAGMA_PASTE: u8 = 99;
pub const T_I_VOID_PEARL: u8 = 100;
pub const T_I_VOID_EYE: u8 = 101;
pub const T_I_FURNACE: u8 = 102;
pub const T_I_BED: u8 = 103;
pub const T_I_PISTON: u8 = 104;
pub const T_I_ENCHANT: u8 = 105;
pub const T_I_FENCE: u8 = 106;
...
fn palette_for(tile: u8) -> [(u8, u8, u8); 16] {
    ...
    match tile {
        ...
        T_I_COAL..=T_I_BOW | T_I_BUCKET..=ICON2_LAST => {
            if let Some(icon) = icon_of(tile) {
                let mut i = 0;
                while i < 8 {
                    p[i + 1] = icon.pal[i];
                    i += 1;
...
fn texel(tile: u8, x: usize, y: usize) -> u8 {
    ...
    let yi = y as i32;
    match tile {
        T_I_COAL..=T_I_BOW | T_I_BUCKET..=ICON2_LAST => match icon_of(tile) {
            Some(icon) => {
                let b = icon.px[y * 8 + x / 2];
                if x & 1 == 0 {
                    b & 0x0F
                } else {
                    b >> 4
...
const fn icon(pal: [(u8, u8, u8); 8], rows: [&[u8; 16]; 16]) -> Icon {
    let mut px = [0u8; 128];
    let mut y = 0;
    while y < 16 {
        let mut x = 0;
...
/// The icon a tile shows, if it is one of the two icon sets.
fn icon_of(tile: u8) -> Option<&'static Icon> {
    match tile {
        T_I_COAL..=T_I_BOW => Some(ICONS[(tile - T_I_COAL) as usize]),
        T_I_BUCKET..=ICON2_LAST => {
            Some(ICONS[(T_I_BOW - T_I_COAL + 1 + tile - T_I_BUCKET) as usize])
        }
        _ => None,
    }
}
...
const I_WATER_BUCKET: Icon = icon(
    [
        (56, 56, 62),
        (168, 168, 176),
        (96, 96, 104),
...
const I_POT_STRENGTH: Icon = icon(
    [
        (132, 150, 172),
        (206, 222, 238),
        (170, 40, 44),
